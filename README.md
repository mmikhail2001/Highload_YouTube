...
|Пиковый исходящий трафик (Тбит / с)|64|
...
- **Пиковое потребление трафика** - 60 Тбит / с
	- **Отдача видео (download)** - 54 Тбит / c 
		- Допустим, что пиковое (дневное) потребление в 1.8 раза больше среднего \[[27](https://cloud.mail.ru/public/8Mau/tKmAN3kqb)] (слайд 14), получим `(133 Пб/сутки + 25 Пб/сутки) * 1024 Тб * 1.8 / 86400 * 8 = 27 Тбит / c`
		- Также учтем некоторый **запас** по потреблению трафика - x2. Таким образом, потребление **54 Тбит/c**. 
	- **Загрузка видео (upload)**
		- С учетом дневного пика (x1.8) и запаса (x2) - **0.8 Тбит / c**
